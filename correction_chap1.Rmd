...
$$
où $\bar{y}_s$ est la moyenne de de $y$ dans l'échantillon. Pour calculer le biais de cet estimateur, on procède comme dans la solution de l'exercice 1.4, 
...
\end{eqnarray*}
où $\hat{\bar{y}}_{\rm{HT}}(s)$ est l'estimateur de Horvitz-Thompson pour un certain échantillonn $s.$ On a  $p(s)=1/6$ pour tous les échantillons $s$ de taille $n=2,$ $|s|=2,$ et zéro sinon, la variance devient:
\begin{eqnarray*}
...
On procède de la même façon que dans la preuve de l'exercice 1.3 et on obtient que les probabilités d'inclusion d'ordre un sont égales à  $\pi_k=2/3$ pour tout $k\in U,$ l'estimateur de Horvitz-Thompson de $\bar{y}_U$ est donc égal à:
$$
\hat{\bar{y}}_{\rm{HT}}=\frac{1}{3}\sum_{k\in s}\frac{y_k}{\pi_k}=\frac{1}{2}\sum_{k\in s}y_k=\bar{y}_s. 
$$
Calcul du biais de $\hat{\bar{y}}_{\rm{HT}}$ *avec les indicatrices d'appartenance*:
$$
...
Calcul du biais de $\hat{\bar{y}}_{\rm{HT}}$ *sans les indicatrices d'appartenance*: 
$$
...
\begin{eqnarray*}
    \hat{t}(s_1) &=& \frac{y_1 + y_2}{6p(s_1)} = \frac{48}{6 \times 0.32} = 25 ~\ \textrm{si} ~\ S= s_1 \quad \mbox{avec}\quad p(s_1)=0.32\\
    \hat{t}(s_2) &=& \frac{y_1 + y_3}{6p(s_2)} = \frac{48}{6 \times 0.40} = 20 ~\ \textrm{si} ~\ S= s_2 \quad \mbox{avec}\quad p(s_2)=0.40\\
    \hat{t}(s_3) &=& \frac{y_2 + y_3}{6p(s_3)} = \frac{40}{6 \times 0.28} \approx 23.80 ~\ \textrm{si} ~\ S= s_3 \quad \mbox{avec}\quad p(s_3)=0.28.
\end{eqnarray*}
La distribution de $\hat{t}$ sous $p_1(\cdot)$ est : 
\begin{eqnarray*}
    \hat{t}(s_1) &=& \frac{y_1 + y_2}{6p_1(s_1)}=\frac{48}{6 \times \frac{1}{3}} = 24 ~\ \textrm{si} ~\ S= s_1 \quad \mbox{avec}\quad p(s_1)=1/3\\
    \hat{t}(s_2) &=& \frac{y_1 + y_3}{6p_1(s_2)}=\frac{48}{6 \times \frac{1}{3}} = 24 ~\ \textrm{si} ~\ S= s_2 \quad \mbox{avec}\quad p(s_2)=1/3\\
    \hat{t}(s_3) &=& \frac{y_2 + y_3}{6p_1(s_3)}= \frac{40}{6 \times \frac{1}{3}} = 20 ~\ \textrm{si} ~\ S= s_3 \quad \mbox{avec}\quad p(s_3)=1/3. 
\end{eqnarray*}
Les distributions de l'estimateur $\hat t$ sont différentes selon si on utilise le plan de sondage $p(\cdot)$ ou $p_1(\cdot). $ 
...
\begin{eqnarray*}
    \E (\hat{\bar{y}}_U)=\sum_{s\in \mathcal S}p(s)\hat{\bar{y}}_U(s)=0.32\frac{\left(y_1 + y_2 \right)}{2}  +  0.4\frac{\left(y_1 + y_3 \right) }{2} + 0.28 \frac{\left(y_2 + y_3 \right)}{2}  = 22.88
\end{eqnarray*}
...
3. On obtient
    \begin{eqnarray*}
        \V \left( \hat{\bar{y}}_U \right) - \V \left(\hat{t}\right) = \frac{y_3 (3y_2 - 3y_1 - y_3)}{54}\cdot
    \end{eqnarray*}
...
\begin{eqnarray*}
1 - \pi_k &=& \mathbb P(k \notin S) \\
&=&\mathbb P( k \mbox{ n'est pas sélectionné au 1er tirage})\times  ... \times\mathbb P( k\mbox{n'est pas  sélectionné au m-ème tirage})\\
&=&
[\mathbb P(k ~\ \textrm{n'est pas sélectionné})]^m=(1-p_k)^m,
\end{eqnarray*}
...
$$
\hat{t}_{y\rm{HH}}= \sum_{k\in U} \frac{y_k}{m \, p_k}  R_k,
$$
où $R_k$ est la variable aléatoire égale au nombre de fois que l'unité $k$ a été sélectionnée après $m$ tirages avec remise, $R_k$ suit une loi Binomiale des paramètres $m$ et $p_k,$ donc $\V(R_k)=mp_k(1-p_k).$ Le vecteur $(R_1, \ldots, R_N)^\top$ suit une loi multinomiale (voir proposition 1.3) et $\C(R_k,R_l)=-mp_kp_l,$ $k\neq l\in U.$
Sa variance de l'estimateur de Hansen-Hurwitz est égale à:
\begin{eqnarray*}
\V(\hat{t}_{y\rm{HH}}) &=&\sum_{k\in U}\left(\frac{y_k}{m \, p_k}\right)^2\V(R_k)+\sum_{k\in U}\sum_{l\neq k\in U}\frac{y_k}{m \, p_k}\frac{y_l}{m \, p_l}\C(R_k, R_l).\\
...
\begin{eqnarray*}
\hat{\V}(\hat{t}_{y\rm{HH}}) = \frac{1}{m(m-1)}\left(\sum_{k\in U}\left(\frac{y_k}{p_k}\right)^2R_k-m\,\hat{t}^2_{y\rm{HH}}\right)
\end{eqnarray*}
et donc, 
\begin{eqnarray*}
\E(\hat{\V}(\hat{t}_{y\rm{HH}})) &=& \frac{1}{m(m-1)}\left(\sum_{k\in U}\left(\frac{y_k}{p_k}\right)^2\E(R_k)-m\,\E(\hat{t}^2_{y\rm{HH}})\right)\\
&=& \frac{1}{m-1}\left(\sum_{k\in U}\left(\frac{y_k}{p_k}\right)^2p_k-t^2_{yU}\right)-\frac{\V(\hat{t}_{y\rm{HH}})}{m-1}\\
&=& \frac{m\V(\hat{t}_{y\rm{HH}})}{m-1}-\frac{\V(\hat{t}_{y\rm{HH}})}{m-1}=\V(\hat{t}_{y\rm{HH}}),
\end{eqnarray*}
car $\E(R_k)=mp_k$ et $\E(\hat{t}^2_{y\rm{HH}})=\V(\hat{t}_{y\rm{HH}})+t^2_{yU}.$
